...
namespace ShamanNoscoSQLService
{
            ...
            try
            {
                using (MySqlConnection cnn = new MySqlConnection(connetionString))
                {
                    using (MySqlCommand cmd = cnn.CreateCommand())
                    {
                        ...
                        cmd.CommandText = "select startdate, clid, dst, recordingfile, agent, IF(finishdate = '0000-00-00 00:00:00', null, finishdate) as finishdate, duration, src from calls " +
                                            "where startdate >= " + DateTime.Now.ToShortDateString() + " and agent  <> 0 and finishdate = '0000-00-00 00:00:00' order by startdate";
                        ...
                        using (MySqlDataReader rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection))
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                foreach (DataRow row in dt.Rows)
                                {
                                    ...
                                    if (!(objRing.Abrir(objRing.GetIDByAgenteId(row["agent"].ToString()).ToString())
                                        && objRing.ANI == row["src"].ToString()))
                                    {
                                        objRing.AgenteId = row["agent"].ToString();
                                        objRing.ANI = row["src"].ToString();
                                        objRing.Campania = row["dst"].ToString();
                                        ...
                                        objRing.GrabacionId = row["recordingfile"].ToString();
